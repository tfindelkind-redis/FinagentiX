...
- Return ONLY valid JSON, no markdown code blocks"""
    ...
    async def _synthesize_with_llm_streaming(
        ...
        Yields chunks of text as they're generated by GPT-4o.
        """
        if not self.openai_client:
            yield json.dumps({"error": "No OpenAI client available"})
            return
        
        try:
            # Extract key metrics (same as non-streaming version)
            current_price = market_data.get("current_price", {}).get("value", "N/A")
            price_date = market_data.get("current_price", {}).get("date", "N/A")
            ...
            # Build streaming-friendly prompt (markdown output instead of JSON)
            prompt = f"""You are a senior investment analyst providing a comprehensive analysis for {ticker}.

## Data Summary
- **Current Price:** ${current_price} (as of {price_date})
- **30-Day Change:** {change_pct}%
- **SMA 50/200:** ${sma_50} / ${sma_200}
- **RSI:** {rsi} | **Volatility:** {volatility}%
- **VaR (95%):** {var_pct}% | **Beta:** {beta} | **Max Drawdown:** {max_drawdown}%
- **Sentiment:** {overall_sentiment} ({json.dumps(sentiment_pcts) if sentiment_pcts else 'N/A'})
- **Rule-Based Signal:** {rule_action} ({', '.join(rule_signals) if rule_signals else 'None'})
...
Provide a comprehensive investment analysis in markdown format:

1. **Executive Summary** (2-3 sentences on the investment case)

2. **Key Insights** (3 bullet points correlating the data)

3. **Risk Considerations** (3 bullet points on risks)

4. **Analysis** (detailed paragraph explaining your reasoning)

5. **Price Context** (brief note on current price vs moving averages)

Be specific with numbers. Acknowledge missing data (N/A). Correlate insights across technical, risk, and sentiment data."""
            ...
            stream = await self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a senior investment analyst. Provide clear, actionable analysis in markdown format. Be concise but insightful."
                    },
            ...
            self.debugger.log_error(exc)
            yield f"\n\n*Error generating analysis: {str(exc)}*"
